...
<ProjectHeader
  ...
  stack={["Python", "React", "TypeScript", "PostgreSQL", "OpenAI Vision API", "Stripe", "Discord OAuth"]}
/>
...
Trilo is a full-stack SaaS I built from the ground up for fantasy football dynasty leagues that run on Discord. It has two parts: a Discord bot that automates league operations (matchups, rosters, records) so commissioners spend minutes instead of hours each week, and a marketing site that handles sign-up, Discord OAuth, Stripe subscriptions, and license delivery. I owned the full product: AI-powered features (like schedule extraction from screenshots), the conversion funnel, and the technical stack end to end.
...
<div className="not-prose my-8">
  <PhoneMockup src="/trilo/game-status.png" alt="Matchup channel showing game status reactions - ✅ (played), 🎲 (simmed), 🟥/🟦 (force win)" />
</div>
